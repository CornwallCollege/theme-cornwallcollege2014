...
class theme_cornwallcollege2014_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = cornwallcollege2014_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        $content = html_writer::start_tag('ul', array('class'=>'dropdown dropdown-horizontal cornwallcollege2014-custom-menu'));
        // Render each child
...
class theme_cornwallcollege2014_topsettings_renderer extends plugin_renderer_base {
            ...
            if($isbranch && $item->children->count()==0) {
                ...
                if (!empty($CFG->navshowallcourses) && $item->key === 'courses') {
                    if(!$coursessubnav) {
                        ...
                        $dummypage = new cornwallcollege2014_dummy_page();
                        $dummypage->set_context($PAGE->context);
                        $dummypage->set_url($PAGE->url);
                        $coursessubnav = new cornwallcollege2014_expand_navigation($dummypage, $item->type, $item->key);
                        $expanded = true;
                    }
                    ...
                    if(!$mainsubnav) {
                        ...
                        $dummypage->set_context($PAGE->context);
                        $dummypage->set_url($PAGE->url);
                        $mainsubnav = new cornwallcollege2014_expand_navigation($dummypage, $item->type, $item->key);
                        $expanded = true;
                    }
                    $subnav = $mainsubnav;
                }
...
class theme_cornwallcollege2014_core_course_renderer extends core_course_renderer {
