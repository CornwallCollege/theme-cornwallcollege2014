YUI.add('moodle-theme_cornwallcollege2014-editbuttons', function(Y) {
...
EditButtons.prototype = {
    ...
    initializer : function(config) {
        ...
        if(!this.editbutton) {
            ...
            this.editbutton.addClass('cornwallcollege2014-editbutton');
        }
        ...
        Y.all('.commands').each(function(icons) {
            ...
            if (!buttons.isEmpty() && buttons.size() == 1) {
                icons.addClass('cornwallcollege2014-noeditbutton');
        ...
        'body', 'a.cornwallcollege2014-editbutton', this);

        try {
            M.core_dock.on('dock:panelresizestart', function(e) {
                var item = M.core_dock.getActiveItem();
                if (item.cornwallcollege2014_editbutton_done) {
                ...
                item.cornwallcollege2014_editbutton_done = true;
            }, M.core_dock.getPanel());

            var attachRemoveHandler = function(item) {
                item.on('dockeditem:itemremoved', function() {
                    var button = this.commands.ancestor('.header').one('.cornwallcollege2014-editbutton-wrap');
                    if (button) button.remove();
                    if (!this.cornwallcollege2014_editbutton_done && this.commands.hasChildNodes()) {
    ...
    wrapButton : function(icons, button) {
        try{
            icons.wrap('<div class="cornwallcollege2014-editbutton-wrap"></div>').insert(button, icons);
        } catch(x) {
            // Fallback for old versions of YUI without Node.wrap() (MDL20)
            var wrapper = Y.DOM.create('<div class="cornwallcollege2014-editbutton-wrap"></div>');
    ...
    toggleButton : function(button) {
        ...
        if(mod) mod.toggleClass('cornwallcollege2014-editbutton-active-module');
    }
};
// Make the colour switcher a fully fledged YUI module
Y.extend(EditButtons, Y.Base, EditButtons.prototype, {
    NAME : 'cornwallcollege2014 theme edit buttoniser',
    ATTRS : {
        // No attributes at present
    }
});
// Our splash theme namespace
M.theme_cornwallcollege2014 = M.theme_cornwallcollege2014 || {};
// Initialisation function for the colour switcher
M.theme_cornwallcollege2014.initEditButtons = function(cfg) {
