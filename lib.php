 ...
 * @return string
 */
function cornwallcollege2014_performance_output($param) {
 ...
 * @return string
 */
function cornwallcollege2014_process_css($css, $theme) {
    ...
    $css = cornwallcollege2014_set_backgroundcolor($css, $backgroundcolor);
    ...
    $css = cornwallcollege2014_set_customcss($css, $customcss);
 ...
 * @return string
 */
function cornwallcollege2014_set_backgroundcolor($css, $backgroundcolor) {
 ...
 * @return string
 */
function cornwallcollege2014_set_customcss($css, $customcss) {
 ...
 *     theme/cornwallcollege2014/yui/editbuttons/editbuttons.js
 *
 * @param moodle_page $page 
 */
function cornwallcollege2014_initialise_editbuttons(moodle_page $page) {
    global $CFG;
    ...
    $page->requires->yui_module('moodle-theme_cornwallcollege2014-editbuttons', 'M.theme_cornwallcollege2014.initEditButtons');
    return true;
}

function cornwallcollege2014_initialise_awesomebar(moodle_page $page) {
    ...
    $page->requires->yui_module('moodle-theme_cornwallcollege2014-awesomebar', 'M.theme_cornwallcollege2014.initAwesomeBar');
}

function cornwallcollege2014_require_course_login($courseorid, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = true) {
    global $CFG, $SITE;
    ...
    if (!empty($CFG->forcelogin)) {
        ...
        cornwallcollege2014_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
...
function cornwallcollege2014_require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
...
class cornwallcollege2014_expand_navigation extends global_navigation {
    ...
    public function expand($branchtype, $id) {
        global $CFG, $DB, $PAGE;
        static $cornwallcollege2014_course_activities = array();
                ...
                try {
                    if(!array_key_exists($course->id, $this->expandedcourses)) {
                        ...
                        if ($PAGE->course->id == $course->id && (!method_exists($this, 'format_display_course_content') || $this->format_display_course_content($course->format))) {
                            cornwallcollege2014_require_course_login($course);
            ...
            case self::TYPE_SECTION :
                ...
                try {
                    ...
                    if (property_exists($PAGE->theme->settings, 'expandtoactivities') && $PAGE->theme->settings->expandtoactivities) {
                        if(!array_key_exists($course->id, $cornwallcollege2014_course_activities)) {
                            list($sectionarray, $activities) = $this->generate_sections_and_activities($course);
                            $cornwallcollege2014_course_activities[$course->id] = $activities;
                        }
                        $sections = $this->expandedcourses[$course->id];
                        $activities = $cornwallcollege2014_course_activities[$course->id];
    ...
    protected function load_section_activities(navigation_node $sectionnode, $sectionnumber, array $activities, $course = null) {
        ...
        foreach ($activities as $activity) {
            ...
            // Legacy onclick removed from cornwallcollege2014 - clicking in Awesomebar should go to the page, not trigger popups etc.
...
class cornwallcollege2014_dummy_page extends moodle_page {
    /**
